...
> **Last updated:** Master Growth & Design Brief completed and approved — see [docs/MASTER_BRIEF_GROWTH_DESIGN.md](MASTER_BRIEF_GROWTH_DESIGN.md). Includes codebase audit, 10 key decisions, 10-tier computable difficulty formula, 1,000-level sawtooth pacing, 2026 ad monetization architecture & eCPM benchmarks, micro-haptics specs, LiveOps/retention schedule, telemetry schema, and 6-month roadmap.
...
128. **Colorblind shape cues map group indices to geometric symbols.** Grouped arrows and gate obstacles render deterministic shape cues (Circle, Square, Triangle, Diamond, Star) so color group relationships don't rely on color perception alone.
129. **7-Day Daily Streak rewards escalate progressively.** `streakStore` tracks consecutive daily claims with rewards escalating from 50 League Arrows on Day 1 to a Golden Chest (500 Arrows + 5 Hints) on Day 7.
130. **Telemetry logs core growth events with zero server overhead.** `telemetry.ts` tracks `level_start`, `level_complete`, `level_fail`, `ad_impression`, `hint_requested`, and `streak_claimed` events safely offline and in dev environments.
131. **Global energy system regenerates 1 heart every 20 minutes.** `energyStore` manages 5 max hearts with offline-safe timestamp-based regeneration and rewarded ad refill option.
...
- **The play screen header has exactly 4 items and no footer.** (1) Circular Back button, (2) Circular Restart button, (3) Centered Tier Name & 3 Hearts, (4) Blue Pill Video Hint button. Bottom action bar and extra chrome are removed.
- **Camera has 0 x-axis margin/padding.** The board extends edge-to-edge horizontally across the screen width (`viewportWidth = width`).
- **Hints cause no camera movement.** Applying a hint directly triggers a minor physical vibration on the hinted arrow without panning the camera.
- **Simulated video ads show interactive modal playback.** Dev ad requests display `DummyAdModal` with video progress before granting hints.
- **Universal Mobile Responsiveness Architecture Refactored.** Added `useResponsive` hook, max content width bounds (`maxWidth: 600`), responsive safe area insets, flexible container constraints, and dynamic board vertical height allocation across all Android and iOS device aspect ratios.
- **Custom 3D League Shield Assets Integrated.** Added 5 custom 3D shield assets (Bronze, Silver, Gold, Ruby/Pink, Diamond/Crystal) across League Mode (`app/(tabs)/leagues.tsx`) and Home Screen (`app/(tabs)/index.tsx`).
- **Collection Tab 3D Award & Trophy Artwork Rendered.** Updated `src/challenge/rewardArt.ts` and `app/(tabs)/collection.tsx` (`AwardTile` & `TrophyTile`) to replace old text glyphs and plain cards with full-fidelity 3D active and locked artwork (`level_legend_active.jpg`, `flame_active.jpg`, `perfect_play_active.jpg`, `trophy_active.jpg`, etc.).
- **Collection Records 3D Artwork & Info Page Navigation Integrated.** Updated `Tile` under the Records section in `app/(tabs)/collection.tsx` to render 3D artwork images (`flame_active.jpg`, `perfect_play_active.jpg`, `level_legend_active.jpg`) and made both Record & Award tiles pressable, opening their dedicated Info Pages (`/challenge/info/longest-streak`, `/challenge/info/perfect-play`, `/challenge/info/level-legend`, `/challenge/info/most-wins`, `/challenge/info/unstoppable`).
- **All 8 Info Pages Standardized to Reference Design.** Created `AwardInfoLayout` (`src/components/AwardInfoLayout.tsx`) featuring 3D artwork frame card with overlapping badge number, earned date pill badge, bold description message, progress track with fill number, and capsule Close button. Applied across all 8 info pages (`league-fighter`, `league-climber`, `level-legend`, `perfect-play`, `unstoppable`, `longest-streak`, `highest-win-streak`, `most-wins`).
- **Records Section 3D Image Assignments Aligned.** Updated the three Record tiles in `app/(tabs)/collection.tsx`: (1) **Longest streak** uses 3D Flame artwork (`flame_active.jpg`), (2) **Highest win streak** uses 3D Crown artwork (`crown_active.jpg`), (3) **Most wins** uses 3D Winged Arrow with Feather artwork (`arrow_active.jpg`).
- **Award Section 3D Artwork Sequence Updated.** Updated award definitions and image artwork mapping in `src/challenge/rewards.ts`, `src/challenge/rewardArt.ts`, and `app/(tabs)/collection.tsx`: (1) **Level Legend** uses 3D Stars (`level_legend_active.jpg`), (2) **Perfect Play** uses 3D Target / Aim Dot (`perfect_play_active.jpg`), (3) **Unstoppable** uses 3D Skull Shield (`unstoppable_active.jpg`), (4) **League Climber** uses 3D Bronze Shield (`league_climber_active.jpg`), (5) **League Fighter** uses 3D Boxing Gloves (`league_fighter_active.jpg`), (6) **League Winners** uses 3D Gold Trophy (`trophy_active.jpg`).
- **Arrow Theme Added & Challenge Screen Top Trophy Restored.** Created new `'arrow'` theme (`src/theme/themes.ts`) with crisp white primary background (`#FFFFFF`), gray secondary borders/surfaces (`#E2E8F0` / `#64748B`), and vibrant purple accents (`#8B5CF6`). On Challenge Mode home (`app/(tabs)/challenge/index.tsx`), retained the 3D Gold/Silver Trophy header icon and Monthly Trophy detail modal on top, while keeping the bottom Record Badges and Awards grids off the page (housed in the Collection tab).
- **Hint Effect Scoped to Minor Vibration Only.** Updated `src/render/SkiaBoard.tsx` so applying a hint triggers *only* a brief physical shiver vibration (`shake.value` sequence) on the target arrow. Stripped glow overlays and scale pulsing per explicit directive.
- **Scene Layout Ratio Configured to 5% Header / 95% Arrow Board & Footer Removed.** Updated `app/play/[id].tsx` so the top Header HUD occupies exactly 5% of screen height (`headerWrap`), the Arrow Board scene occupies 95% of screen height (`boardWrap` & `boardAreaHeight`), and removed bottom pan hints / footer buttons.
- **Hint Camera Panning & Arrow Vibration Combined.** Updated `applyHint` in `app/play/[id].tsx` to calculate board coordinates of the hinted arrow's head (`arrow.body[0]`) and dispatch camera focus (`setFocusX`, `setFocusY`, `setFocusNonce`), smoothly panning the camera to center on the target arrow while executing the minor physical vibration shiver.
- **Board Background Configured to Pure White.** Updated `arrowPalette.board` in `src/theme/themes.ts` to pure White (`#FFFFFF`) matching the clean white theme background.
- **Home Screen Title & Layout Centered.** Updated `app/(tabs)/index.tsx` to align the `▲ rrows` wordmark in a horizontal flex container (`wordmarkRow`), vertically and horizontally centering the purple triangle icon with the brand text, level status, and play button.
- **Level Legend Info Page Aligned to Reference Screenshot.** Updated `AwardInfoLayout` (`src/components/AwardInfoLayout.tsx`) and `level-legend.tsx` to render a 3D circular graphic badge with overlapping `"500"` number, date pill `"Aug 3 2026"`, centered bold description message, green progress bar with number `"601"`, and outline capsule Close button perfectly centered on screen.
- **Most Wins Info Page Aligned to Reference Screenshot.** Updated `most-wins.tsx` and `AwardInfoLayout.tsx` to render 3D Winged Arrow with overlapping `"94"` badge number, date pill `"Jul 11 2026"`, centered bold description `"You won 94 levels in a day!"`, `"Current"` status pill, and `"You won 0 levels today."` stats text.
- **Hint Arrow Vibration Duration Set to 3 Seconds.** Updated `shake.value` animation loop in `src/render/SkiaBoard.tsx` to repeat the minor physical vibration shiver sequence for 15 cycles (3,000ms / 3 seconds).
- **Removed Regular, Committed, Sharp Eye, and Unbroken Awards.** Updated `REWARDS` array in `src/challenge/rewards.ts` and route map in `app/(tabs)/collection.tsx` to remove `Regular`, `Committed`, `Sharp Eye`, and `Unbroken` awards. Retained the 6 core awards (**Level Legend**, **Perfect Play**, **Unstoppable**, **League Climber**, **League Fighter**, and **League Winners**).
- **Collection Screen Tile Container Backgrounds Set to Transparent.** Updated `Tile`, `AwardTile`, and `TrophyTile` in `app/(tabs)/collection.tsx` to use transparent background and border styles (`backgroundColor: 'transparent'`, `borderColor: 'transparent'`) with `resizeMode="contain"` across all images.
- **Level Legend Configured to 11 Tier Milestones.** Updated `app/(tabs)/challenge/info/level-legend.tsx` with 11 level milestone thresholds (`Level 5`, `100`, `200`, `300`, `400`, `500`, `600`, `700`, `800`, `900`, `1000`). Automatically calculates highest achieved milestone and next target milestone.
- **Dynamic Tier Milestones Applied Across All Records & Awards.** Updated all info screens (`longest-streak.tsx`, `highest-win-streak.tsx`, `perfect-play.tsx`, `unstoppable.tsx`, `league-climber.tsx`, `league-fighter.tsx`, `most-wins.tsx`) to dynamically track milestone progression arrays (e.g., streak milestones `3, 7, 14, 30... 365`, win streak milestones `5, 10, 25... 500`, flawless milestones `1, 5, 10... 100`, promotion milestones `1, 3, 5... 50`), rendering the player's peak record, current progress, and next milestone target.
- **Integrated 6 High-Res 3D Metallic Shield Assets Across Leagues Tab.** Updated `assets/images/` and `LEAGUE_IMAGES` in `app/(tabs)/leagues.tsx` & `app/(tabs)/index.tsx` to render all 6 high-res 3D metallic division shields (**Bronze**, **Silver**, **Gold**, **Rose Gold / Ruby**, **Obsidian**, and **Diamond**), including rendering the 6 overlapping 3D shields row in the Leagues info modal matching reference design.
- **Home Screen 3D Dark Arrow Icon & Play Button Aligned.** Updated `app/(tabs)/index.tsx` to use the 3D dark matte arrow icon for the Leagues card score pill (with transparent background) and aligned the Home Play button dimensions and typography (`height: 60`, `width: '85%'`, `fontSize: 22`, `letterSpacing: 1`) to be in exact visual alignment with the Challenge screen Play button.
- **3D Dark Arrow Asset Background Converted to Pure Transparent PNG.** Processed `assets/images/arrow_icon.png` to strip all white background pixels, producing a 100% clean transparent PNG asset for the Leagues card score pill.
- **Board Arrows Set to Solid Black.** Updated `arrowPalette` in `src/theme/themes.ts` so `arrow: '#18181B'` (solid matte black, zero purple), `arrowShadow: 'rgba(0, 0, 0, 0.20)'`, and `arrowHinted: '#3F3F46'`. All gameplay board arrows now render in clean solid black.
- **Dynamic 50-Player League Leaderboard Starting at Bronze Implemented.** Updated `app/(tabs)/leagues.tsx` to replace static mock list with a dynamic 50-player roster starting in **Bronze League** (`0` initial arrows). Player score updates in real time, dynamically sorting rank (1 to 50) with visual dividers for **PROMOTION ZONE (TOP 10)** and **DEMOTION ZONE (BOTTOM 5)**.
- **Android APK Built Successfully.** Compiled native release-ready Android debug build via `./gradlew assembleDebug`. Output APK saved at `d:\arrow-escape-game\ArrowPath.apk` (and `android/app/build/outputs/apk/debug/app-debug.apk`).
- **Wrong-Arrow Tap Animation, Hint Feedback, Box Obstacle Removal & Animation Audit Implemented.** (1) Overhauled blocked arrow animation in `src/render/SkiaBoard.tsx` with smooth 100ms forward lurch toward blocker and 150ms return to rest position (`0`), worklet reset `shake.value = 0`, and auto-clearing `state.highlight` in `app/play/[id].tsx`. (2) Wired `Vibration.vibrate(80)` fallback for universal Android hardware support during hint activation alongside `Haptics.impactAsync`. (3) Completely removed box obstacle (`walls`) logic from ray casting in `board.ts`, solver in `solver.ts`, scene building in `scene.ts`, and Skia rendering in `skiaScene.ts` while preserving arrow-to-arrow blocking and gate mechanics intact. (4) Added rapid-tap protection in `handleTap` ignoring taps on departing/escaping arrows and non-playing game states.
...
128. **Blocked arrow taps move forward to the collision point and trigger impact haptics at the exact collision moment.** A blocked tap moves the arrow forward along its exit vector by `freeCells * cellSize` (or 0.25 cells for an adjacent blocker). At the exact collision impact, `onCollisionHit` triggers physical device vibration (`triggerCollisionHaptic`) and switches the rendered color to `palette.arrowBlocked`, before returning smoothly to its original position and restoring normal color. Hint haptics (`triggerHintHaptic`) remain strictly isolated.
