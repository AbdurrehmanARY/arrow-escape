 ...
 *
 * Purpose:      Turn the 1,000-level curriculum into packs of verified levels.
 * Responsibilities:
 ...
 *
 *               Written as **packs of 50 rather than one file per level.** Metro
 *               charges real overhead per module, and a thousand JSON modules is a
...
// Generating a thousand levels takes many minutes, and clearing the directory
// first left the app unbuildable for that whole window — Metro cannot resolve
// `@data/levels` when it is empty, so a rebuild broke the running dev server
// every time. Stale packs are swept at the end instead; see below.
 ...
 * ${results.length} of them cost ${packNames.length} Metro modules rather than ${results.length}. Decoding happens
